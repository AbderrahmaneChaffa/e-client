...
@section('page-title','Détails facture')

@section('content')
<div class="max-w-4xl mx-auto">
    <div class="mb-4">
        <a href="{{ route('client.factures.index') }}" class="text-blue-600 hover:underline text-sm">
            <i class="fa-solid fa-arrow-left mr-1"></i> Retour à mes factures
        </a>
    </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden mb-6">
        <div class="p-6 border-b border-gray-50 flex justify-between items-start">
            <div>
                ...
                <p class="text-gray-500">Émise le {{ $facture->date_facture->format('d/m/Y') }}</p>
            </div>
            <div class="text-right">
                <span class="px-4 py-2 rounded-full text-sm font-bold {{ $facture->reste_a_payer <= 0 ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700' }}">
                    {{ $facture->reste_a_payer <= 0 ? 'Payée' : 'Solde dû' }}
                </span>
            </div>
        </div>

        <div class="grid grid-cols-1 md:grid-cols-2 gap-8 p-6 bg-gray-50/50">
            <div>
                <h3 class="text-xs uppercase font-bold text-gray-400 mb-2">Navire</h3>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100">
        <div class="p-4 border-b font-bold bg-green-50 text-green-800">Historique des paiements</div>
        ...
        <table class="w-full text-left">
            <thead>
                <tr class="text-xs font-bold text-gray-400 uppercase border-b">
                    ...
                    <th class="p-4 text-right text-green-700">Montant</th>
                </tr>
            </thead>
            <tbody class="text-sm">
        ...
        @else
        <div class="p-8 text-center text-gray-400 italic text-sm">Aucun paiement enregistré.</div>
        @endif
    </div>
</div>
@endsection
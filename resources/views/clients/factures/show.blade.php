...
<div class="space-y-6">
    ...
    >
        ...
        @if(! $facture->annuler)
            <a href="{{ route('client.invoices.facture.print', $facture) }}" target="_blank" class="ui-btn-primary">
                <i data-lucide="download" class="h-4 w-4" aria-hidden="true"></i>
                Facture PDF
            </a>
        @endif
        <button type="button" class="ui-btn-secondary" x-data @click="$dispatch('open-modal', 'support-modal')">
        ...
        <main class="space-y-6 xl:col-span-2">
            <article class="ui-card overflow-hidden">
                ...
                @if($facture->prestations->isEmpty())
                    <div class="p-4"><x-empty-state icon="list" title="Aucune ligne" message="Aucune prestation detaillee n'est disponible." /></div>
                @else
                        ...
                        @foreach($facture->prestations as $prestation)
                            <div class="flex flex-col gap-3 px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
                                <div>
                                    ...
                                    <p class="text-sm text-gray-500 dark:text-gray-400">{{ $prestation->article }} · Qte {{ number_format($prestation->quantite, 0, ',', ' ') }}</p>
                                </div>
            ...
            <article class="ui-card overflow-hidden">
                ...
                @if($facture->paiements->isEmpty())
                    <div class="p-4"><x-empty-state icon="credit-card" title="Aucun paiement" message="Aucun paiement n'est encore associe a cette facture." /></div>
                @else
                    <div class="divide-y divide-gray-200 dark:divide-gray-700">
                        @foreach($facture->paiements->sortByDesc('date_paiement') as $payment)
                            <div class="flex items-center justify-between gap-3 px-5 py-4">
                                <div>
                                    ...
                                    <p class="text-sm text-gray-500 dark:text-gray-400">{{ $payment->date_paiement ? \Illuminate\Support\Carbon::parse($payment->date_paiement)->format('d/m/Y') : '-' }} · {{ $payment->banque ?? '-' }}</p>
                                </div>
                                <p class="font-bold text-success-700 dark:text-success-300">{{ number_format($payment->montant, 2, ',', ' ') }} DA</p>
                            </div>
                        @endforeach
                    </div>
                @endif
        ...
        <aside class="space-y-6">
            <article class="ui-card p-5">
                ...
                <dl class="mt-4 space-y-3 text-sm">
                    ...
                    <div class="flex justify-between gap-4 border-t border-gray-200 pt-3 dark:border-gray-700"><dt>Total TTC</dt><dd class="text-lg font-bold">{{ number_format($facture->total_ttc, 2, ',', ' ') }} DA</dd></div>
            ...
            <article class="ui-card p-5">
                ...
                <dl class="mt-4 space-y-3 text-sm">
                    ...
                    <div><dt class="text-gray-500 dark:text-gray-400">Escale</dt><dd class="font-medium">{{ $facture->escale?->numero_escale ?? '-' }}</dd></div>
                    <div><dt class="text-gray-500 dark:text-gray-400">Poste quai</dt><dd class="font-medium">{{ $facture->escale?->poste_quai ?? '-' }}</dd></div>
                </dl>
            </article>
        </aside>
    </div>
    ...
    <x-modal id="support-modal" title="Contacter le support">
        <form class="space-y-4">
            <div>
                ...
                <input id="support_subject" class="ui-input" value="Question sur la facture #{{ $facture->numero_facture }}">
            </div>
            <div>
                <label for="support_message" class="ui-label mb-1">Message</label>
                <textarea id="support_message" rows="4" class="ui-input" placeholder="Decrivez votre demande"></textarea>
            </div>
            <div class="flex justify-end gap-2">
                <button type="button" class="ui-btn-secondary" x-on:click="$dispatch('close-modal', 'support-modal')">Annuler</button>

...
@section('content')
    <div x-data="importUploader()" x-init="init()" class="space-y-6">
        <x-page-header title="Imports ERP BIG"
        ...
        <section class="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
            <x-stat-card title="Imports visibles" :value="number_format($batches->count(), 0, ',', ' ')" icon="upload-cloud"
                color="primary" />
            <x-stat-card title="Termines" :value="number_format($statusCounts->get('completed', 0), 0, ',', ' ')"
                icon="check-circle-2" color="success" />
            ...
            <x-stat-card title="Echecs" :value="number_format($statusCounts->get('failed', 0), 0, ',', ' ')"
                icon="circle-alert" color="danger" />
        </section>
        ...
        <section class="ui-card p-5">
            <div class="grid grid-cols-1 gap-6 xl:grid-cols-3">
                <div class="xl:col-span-2">
                    <div class="mb-4">
                        <h2 class="text-base font-semibold text-gray-900 dark:text-gray-100">
                            Nouvel import</h2>
                        <p class="text-sm text-gray-600 dark:text-gray-400">Selectionnez vos
                            fichiers Excel dans n'importe quel ordre.</p>
                    </div>

                    <div class="rounded-lg border-2 border-dashed border-primary-200 bg-primary-50/60 p-6 text-center transition hover:border-primary-400 dark:border-primary-900/60 dark:bg-primary-900/10"
                        @dragover.prevent @drop.prevent="setDroppedFiles($event)">
                        <input x-ref="filesInput" type="file" class="hidden" accept=".xlsx,.xls" multiple
                            @change="setFiles($event)">
                        <div
                            class="mx-auto mb-4 flex h-14 w-14 items-center justify-center rounded-full bg-white text-primary-600 shadow-sm dark:bg-gray-800 dark:text-primary-300">
                            <i data-lucide="file-spreadsheet" class="h-7 w-7" aria-hidden="true"></i>
                        </div>
                        <p class="font-semibold text-gray-900 dark:text-gray-100">
                            Glissez-deposez vos fichiers ici</p>
                        <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">Formats
                            acceptes: .xlsx, .xls · maximum 5 fichiers</p>
                            ...
                            <template x-for="file in files" :key="file.name + file.size">
                                <div class="rounded-lg border border-gray-200 p-3 dark:border-gray-700">
                                    <div class="flex items-center gap-3">
                                        <i data-lucide="file-spreadsheet" class="h-5 w-5 text-success-600"
                                            aria-hidden="true"></i>
                                        <div class="min-w-0">
                                            <p class="truncate text-sm font-semibold" x-text="file.name"></p>
                                            <p class="text-xs text-gray-500 dark:text-gray-400"
                                                x-text="formatBytes(file.size)"></p>
                                        </div>
                                    </div>
                                </div>
                            </template>
                ...
                <aside class="space-y-4">
                    <div
                        class="rounded-lg border border-info-200 bg-info-50 p-4 text-info-800 dark:border-info-900/60 dark:bg-info-900/20 dark:text-info-200">
                        <div class="flex items-start gap-3">
                            <i data-lucide="info" class="mt-0.5 h-5 w-5 shrink-0" aria-hidden="true"></i>
                            <p class="text-sm">Ordre garanti: Factures, Prestations, Paiements,
                                Factures Payees, Prestations Payees.</p>
                        </div>
                    </div>
                    ...
                    <label
                        ...
                        <input type="checkbox" x-model="forceImport"
                            class="rounded border-gray-300 text-primary-600 focus:ring-primary-600 dark:border-gray-700 dark:bg-gray-900">
                        Forcer l'import meme si un doublon est detecte
                    </label>

                    <div class="grid grid-cols-1 gap-3">
                        <button type="button" class="ui-btn-secondary w-full" :disabled="!files.length || processing"
                            @click="preview()">
                            <i data-lucide="scan-search" class="h-4 w-4" aria-hidden="true"></i>
                            Previsualiser
                        </button>
                        <button type="button" class="ui-btn-primary w-full" :disabled="!files.length || processing"
                            @click="startImport()">
                            <i data-lucide="loader-circle" x-show="processing" x-cloak class="h-4 w-4 animate-spin"
                                aria-hidden="true"></i>
            ...
            <div x-show="message" x-cloak class="mt-5 rounded-lg border p-4 text-sm"
                :class="messageType === 'error' ? 'border-danger-200 bg-danger-50 text-danger-800 dark:border-danger-800 dark:bg-danger-900/20 dark:text-danger-200' : 'border-success-200 bg-success-50 text-success-800 dark:border-success-800 dark:bg-success-900/20 dark:text-success-200'">
                <p x-text="message"></p>
            </div>
                            ...
                            <div class="flex items-start justify-between gap-3">
                                <div class="min-w-0">
                                    <p class="truncate font-semibold"
                                        x-text="row.filename || row.original_filename || 'Fichier'">
                                    </p>
                                    <p class="text-sm text-gray-500 dark:text-gray-400"
                                        x-text="row.type || row.detected_type || 'Type detecte'">
                                    </p>
                                </div>
                                <span
                                    ...
                                    x-text="row.rows || row.total_rows || 'OK'"></span>
                            </div>
                        </div>
                    </template>
                </div>
                ...
                <template x-for="item in Object.values(progress)" :key="item.id">
                    <div class="rounded-lg border border-gray-200 p-4 dark:border-gray-700">
                        ...
                        <p class="mt-2 text-xs text-gray-500 dark:text-gray-400"
                            x-text="`${item.processed || 0}/${item.total || 0} lignes · ${item.eta || 'ETA indisponible'}`">
                        </p>
                    </div>
                </template>
            </div>
        ...
        <form method="GET" action="{{ route('admin.imports.index') }}" class="ui-card p-4">
            <div class="grid grid-cols-1 gap-3 lg:grid-cols-12">
                ...
                <div class="lg:col-span-2">
                    <select name="per_page" class="ui-input" aria-label="Entrees">
                        @foreach([10, 20, 50, 100] as $size)
                            <option value="{{ $size }}" @selected((int) request('per_page', 20) === $size)>{{ $size }} / page
                            </option>
                        @endforeach
                    </select>
                </div>
                <div class="flex gap-2 lg:col-span-2">
                    ...
                    @if($activeFilters)
                        <a href="{{ route('admin.imports.index') }}" class="ui-btn-secondary"><i data-lucide="rotate-ccw"
                                class="h-4 w-4"></i></a>
                    @endif
                </div>
            </div>
        ...
        <section class="ui-card overflow-hidden">
            @if($batches->isEmpty())
                <div class="p-4">
                    <x-empty-state icon="upload-cloud" title="Aucun import"
                        message="L'historique des imports apparaitra ici apres le premier depot." />
                </div>
            @else
                <div class="hidden md:block">
                    <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                        <thead class="bg-gray-50 dark:bg-gray-900/60">
                            <tr>
                                <th scope="col" class="px-4 py-3 text-left text-xs font-semibold uppercase text-gray-500">
                                    Fichier</th>
                                <th scope="col" class="px-4 py-3 text-left text-xs font-semibold uppercase text-gray-500">
                                    Type</th>
                                <th scope="col" class="px-4 py-3 text-left text-xs font-semibold uppercase text-gray-500">
                                    Statut</th>
                                <th scope="col" class="px-4 py-3 text-right text-xs font-semibold uppercase text-gray-500">
                                    Lignes</th>
                                <th scope="col" class="px-4 py-3 text-left text-xs font-semibold uppercase text-gray-500">
                                    Cree par</th>
                                <th scope="col" class="px-4 py-3 text-left text-xs font-semibold uppercase text-gray-500">
                            ...
                            @foreach($batches as $batch)
                                <tr class="transition-colors hover:bg-gray-50 dark:hover:bg-gray-700/50">
                                    ...
                                    <td class="px-4 py-4 text-sm">
                                        {{ ucfirst(str_replace('_', ' ', $batch->type)) }}
                                    </td>
                                    ...
                                    <td class="px-4 py-4 text-sm">{{ $batch->creator?->name ?? '-' }}
                                    </td>
                                    <td class="px-4 py-4 text-sm">
                                        {{ optional($batch->created_at)->format('d/m/Y H:i') }}
                                    </td>
                                    <td class="px-4 py-4 text-sm">
                                        {{ $batch->completed_at ? $batch->created_at->diffForHumans($batch->completed_at, ['parts' => 2]) : '-' }}
                                    </td>
                                    <td class="px-4 py-4 text-sm">
                                    ...
                                    <td class="px-4 py-4">
                                        <div class="flex justify-end gap-2">
                                            <a href="{{ route('admin.imports.show', $batch) }}" class="ui-icon-btn"
                                                aria-label="Voir details"><i data-lucide="eye" class="h-4 w-4"></i></a>
                                            @if($batch->status !== 'processing')
                                                <button type="button" class="ui-icon-btn" @click="resume({{ $batch->id }})"
                                                    aria-label="Relancer l'import"><i data-lucide="refresh-cw"
                                                        class="h-4 w-4"></i></button>
                                            @endif
                                            @if(in_array($batch->status, ['completed', 'failed', 'pending'], true))
                                                <button type="button" class="ui-icon-btn text-danger-600 dark:text-danger-300"
                                                    @click="destroy({{ $batch->id }})" aria-label="Supprimer l'import"><i
                                                        data-lucide="trash-2" class="h-4 w-4"></i></button>
                                            @endif
                                        </div>
                                    </td>
                                </tr>
                ...
                <div class="space-y-3 p-4 md:hidden">
                    @foreach($batches as $batch)
                        <article class="rounded-lg border border-gray-200 p-4 dark:border-gray-700">
                            <div class="flex items-start justify-between gap-3">
                                <div class="min-w-0">
                                    <p class="truncate font-semibold">{{ $batch->original_filename }}
                                    </p>
                                    <p class="text-sm text-gray-500 dark:text-gray-400">
                                        {{ ucfirst(str_replace('_', ' ', $batch->type)) }}
                                    </p>
                            ...
                            <p class="mt-2 text-xs text-gray-500 dark:text-gray-400">
                                {{ number_format($batch->processed_rows, 0, ',', ' ') }} /
                                {{ number_format($batch->total_rows, 0, ',', ' ') }} lignes
                            </p>
                        </article>
                    @endforeach
                </div>
